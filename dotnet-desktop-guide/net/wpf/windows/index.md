...
In this example `Window1` is instantiated when the application starts, which occurs when the <xref:System.Windows.Application.Startup> event is raised in which the `Application_Startup` method is called. For more information about the startup window, see [How to get or set the main application window](how-to-get-set-main-application-window.md).
